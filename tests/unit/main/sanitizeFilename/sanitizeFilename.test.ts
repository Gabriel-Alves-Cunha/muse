import { readFile, unlink, writeFile, mkdtempSync, rm, rmSync } from "fs";
	...
	() => {
		...
		describe("Test writing files to the local filesystem.", () => {
			function testStringUsingFS(str: string) {
				...
				const filepath = join(tempdir, sanitized);
				...
				expect(dirname(normalize(filepath))).toBe(tempdir);

				writeFile(filepath, "foobar", (err) => {
					if (err) throw err;

					readFile(filepath, (err, data) => {
						if (err) throw err;

						expect(data.toString(), "File should not contain equals").toBe(
							"foobar",
						);
